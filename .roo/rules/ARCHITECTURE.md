...
│   ├── Batch/                # Batch Send engine (stress testing)
│   │   ├── Models/           # WorkflowStep, PatientContext, BatchWorkflow, BatchSendResult
│   │   ├── BatchSendEngine.cs
│   │   ├── BatchWorkflowTemplates.cs
│   │   └── PatientDataRandomizer.cs
│   └── UpdateChecker.cs      # GitHub release update checking
...
- Simple final summary per patient (not real-time progress)
...
| File | Changes |
|------|---------|
| `MauiProgram.cs` | Added DI registrations for BatchSendEngine, PatientDataRandomizer, BatchSendViewModel, BatchSendPage |
| `AppShell.xaml` | Added `ShellContent` with `Route="BatchSendPage"` |
| `NetworkSettingsPage.xaml` | Added "Batch Send" button in HL7 Tools card |
| `NetworkSettingsViewModel.cs` | Added `OpenBatchSendCommand` → `GoToAsync("//BatchSendPage")` |
| `HL7Tester.csproj` | Version incremented to 2.0.19 |
| `Platforms/Windows/app.manifest` | Version incremented to 2.0.19.0 |
| `Platforms/Windows/Package.appxmanifest` | Version incremented to 2.0.19.0 |

---

*Last Updated: September 25, 2026 (v2.0.19)*
